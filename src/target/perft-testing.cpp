...
void EvaluatePossiblePosition(const BitWiseBoard &board,
                              std::list<PositionalInfo> &new_positions) {
...
std::list<PositionalInfo>
GetAllPossiblePositions(const std::list<PositionalInfo> &positions) {
  std::list<PositionalInfo> new_positions;
...
void EvaluateFen(size_t depth, std::string fen,
                 const std::vector<size_t> &quantity, const std::string name) {
  if (!std::filesystem::is_directory("./moves-test")) {
    std::filesystem::create_directory("./moves-test");
  }
  if (depth > quantity.size()) {
    std::cout << "there isnt enough data to test if it actually captures what "
                 "you want :(\n";
  }
  std::list<PositionalInfo> positions = {
      PositionalInfo{.board = api.BuildFromFEN(fen),
                     .from = {0, 0},
                     .to = {0, 0},
                     .previous_fen = ""}};
  ...
  std::cout << "Name:            " << name << "\n";
  std::cout << "-----------------------------------\n";
  std::cout << "Depth: " << 0 << " Result:" << positions.size()
            << " positions\n";
  ...
  for (size_t epoch = 0; epoch < depth; epoch++) {
    ...
    std::cout << "Depth: " << epoch + 1 << " Result:" << positions.size()
              << " positions | passed: " << (passed ? "[OK]" : "[FAIL]")
              << " expected:" << quantity[epoch] << " duration: "
    ...
    //{
    //
    //    file << api.GetFen(info.board) << "," <<
    //    BoardCoordinate2String(info.from) << "," <<
    //    BoardCoordinate2String(info.to) << "," << info.previous_fen << "\n";
    //}
    // file.close();
  }
  std::cout << "-----------------------------------\n\n";
...
void GeneralEvaluation() {
  ...
  std::array<Eval_t, 6> fen_eval = {
      Eval_t{.fen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",
      ...
      Eval_t{
          .fen = "r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/1PP1QPPP/"
                 "R4RK1 w - - 0 10",
