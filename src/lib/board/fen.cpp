...
void BoardAPI::buildBoardPiecePlacementFen(BitWiseBoard &board, const std::string &fen_pieces)
{
    int8_t x = 0;
    int8_t y = 0;
...
void BoardAPI::selectColorFen(BitWiseBoard &board, const std::string &fen_color)
{
    board.white_to_move = fen_color == "w";
}
void BoardAPI::castlingRightsFen(BitWiseBoard &board, const std::string &fen_castling)
{
    // castling rights
...
void BoardAPI::enPassantFen(BitWiseBoard &board, const std::string &fen_en_passant)
{
...
void BoardAPI::fenFromBoardPieces(const BitWiseBoard &board, std::string &fen_pieces)
{
    for (int8_t y = 0; y < 8; y++)
    {
        ...
        {
            TypePiece piece = GetPieceFromCoord(BoardCoordinates{.x = x, .y = y}, board);
...
void BoardAPI::fenFromCastlingRights(const BitWiseBoard &board, std::string &fen_castling)
{
    int can_castle = 0;
...
std::string BoardAPI::GetFen(const BitWiseBoard &board)
{
    ...
    return std::format("{} {} {} {} {} {}",
                       output.piece_placement,
                       output.active_color,
                       output.castling_availability,
                       output.en_passant_target_square,
