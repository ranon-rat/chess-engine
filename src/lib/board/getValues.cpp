...
bool BoardAPI::EnemySquares(BoardCoordinates from, const BitWiseBoard &board,
                            bool is_white) {
...
bool BoardAPI::FriendSquares(BoardCoordinates from, const BitWiseBoard &board,
                             bool is_white) {
  uint64_t piece_mask = 1ULL << ((from.y * 8) + from.x);
...
bool BoardAPI::OcuppiedSquares(BoardCoordinates from,
                               const BitWiseBoard &board) {
  uint64_t piece_mask = 1ULL << ((from.y * 8) + from.x);
  return (piece_mask & GetOccupiedSquares(board));
}
TypePiece BoardAPI::GetPieceFromCoord(BoardCoordinates from,
                                      const BitWiseBoard &board) {
  uint64_t piece_mask = 1ULL << ((from.y * 8) + from.x);
...
uint64_t BoardAPI::GetOccupiedSquares(const BitWiseBoard &board) {
