    ...
    for (int i = 0; i < 8; i++) {
      ...
      if ((i + j) % 2 == 0)
        DrawRectangle(i * SQUARE_SIZE, j * SQUARE_SIZE, SQUARE_SIZE,
                      SQUARE_SIZE, DARKGRAY);
      else
        DrawRectangle(i * SQUARE_SIZE, j * SQUARE_SIZE, SQUARE_SIZE,
                      SQUARE_SIZE, LIGHTGRAY);
    }
  }
}
  ...
  for (int j = 0; j < 8; j++) {
    for (int i = 0; i < 8; i++) {
      ...
      case Pieces::PAWN:
        ...
        DrawRectangle(i * SQUARE_SIZE + 10, j * SQUARE_SIZE + 10, 30, 30,
                      piece.isWhite ? WHITE : BLACK);
        DrawText("P", i * SQUARE_SIZE + 20, j * SQUARE_SIZE + 20, 20,
                 piece.isWhite ? BLACK : WHITE);
      ...
      case Pieces::KNIGHT:
        ...
        DrawRectangle(i * SQUARE_SIZE + 10, j * SQUARE_SIZE + 10, 30, 30,
                      piece.isWhite ? WHITE : BLACK);
        DrawText("N", i * SQUARE_SIZE + 20, j * SQUARE_SIZE + 20, 20,
                 piece.isWhite ? BLACK : WHITE);
        break;
      case Pieces::BISHOP:
        ...
        DrawRectangle(i * SQUARE_SIZE + 10, j * SQUARE_SIZE + 10, 30, 30,
                      piece.isWhite ? WHITE : BLACK);
        DrawText("B", i * SQUARE_SIZE + 20, j * SQUARE_SIZE + 20, 20,
                 piece.isWhite ? BLACK : WHITE);
        break;
      case Pieces::ROOK:
        ...
        DrawRectangle(i * SQUARE_SIZE + 10, j * SQUARE_SIZE + 10, 30, 30,
                      piece.isWhite ? WHITE : BLACK);
        DrawText("R", i * SQUARE_SIZE + 20, j * SQUARE_SIZE + 20, 20,
                 piece.isWhite ? BLACK : WHITE);
        break;
      case Pieces::QUEEN:
        ...
        DrawRectangle(i * SQUARE_SIZE + 10, j * SQUARE_SIZE + 10, 30, 30,
                      piece.isWhite ? WHITE : BLACK);
        DrawText("Q", i * SQUARE_SIZE + 20, j * SQUARE_SIZE + 20, 20,
                 piece.isWhite ? BLACK : WHITE);
        break;
      case Pieces::KING:
        ...
        DrawRectangle(i * SQUARE_SIZE + 10, j * SQUARE_SIZE + 10, 30, 30,
                      piece.isWhite ? WHITE : BLACK);
        DrawText("K", i * SQUARE_SIZE + 20, j * SQUARE_SIZE + 20, 20,
                 piece.isWhite ? BLACK : WHITE);
        break;
      case Pieces::UNKOWN:
        DrawRectangle(i * SQUARE_SIZE + 10, j * SQUARE_SIZE + 10, 30, 30,
                      piece.isWhite ? WHITE : BLACK);
        DrawText("?", i * SQUARE_SIZE + 20, j * SQUARE_SIZE + 20, 20,
                 piece.isWhite ? BLACK : WHITE);
      default:
        break;
      }
      if (is_attacked) {
        DrawRectangle(i * SQUARE_SIZE + 5, j * SQUARE_SIZE + 5, 40, 40,
                      {.r = 0, .g = 0, .b = 255, .a = SQUARE_SIZE});
      }
    }
  }
}
...
void ChessGame::SelectPieces() {
  ...
  if (we_are_white != bitwise_board.white_to_move) {
    auto m = engine.SelectMovement(bitwise_board);
    if (m.has_value()) {
    ...
    if (currentTime - lastClickTime > clickCooldown) {
      ...
      for (int x = 0; x < 64; x++) {
        if (CheckCollisionPointRec(mousePos, squares[x])) {
          DrawRectangle(squares[x].x, squares[x].y, squares[x].width,
                        squares[x].height, RED);
          ...
          if (not_selected) {
            ...
            to.y = x / 8;
            bitwise_board =
                api.MakeMove(from, to, bitwise_board, TypeGame::User);
            ...
            std::cout << "king in check: "
                      << (bitwise_board.king_check ? "true" : "false") << "\n";
            std::cout << "white pawns      "
                      << bitwise_board.pawns_c[WHITE_TO_MOVE] << "\n";
            std::cout << "black pawns      "
                      << bitwise_board.pawns_c[BLACK_TO_MOVE] << "\n";

            std::cout << "white knights   "
                      << bitwise_board.knights_c[WHITE_TO_MOVE] << "\n";
            std::cout << "black knights   "
                      << bitwise_board.knights_c[BLACK_TO_MOVE] << "\n";

            std::cout << "white bishops   "
                      << bitwise_board.bishops_c[WHITE_TO_MOVE] << "\n";
            std::cout << "black bishops   "
                      << bitwise_board.bishops_c[BLACK_TO_MOVE] << "\n";

            std::cout << "white rooks     "
                      << bitwise_board.rooks_c[WHITE_TO_MOVE] << "\n";
            std::cout << "black rooks     "
                      << bitwise_board.rooks_c[BLACK_TO_MOVE] << "\n";

            std::cout << "white queens    "
                      << bitwise_board.queens_c[WHITE_TO_MOVE] << "\n";
            std::cout << "black queens    "
                      << bitwise_board.queens_c[BLACK_TO_MOVE] << "\n";
...
void ChessGame::ShowBasicInformation() {
  ...
  DrawText(bitwise_board.white_to_move ? "white move" : "black move", x_offset,
           10, 10, BLACK);
}
...
void ChessGame::DrawGameState() {
  ...
  if (game_state == GameStates::CHECKMATE) {
    ...
    DrawText("CHECKMATE BITCH", SQUARE_SIZE * 8 + x_offset, screenHeight / 2,
             font_size, BLACK);
    DrawText(bitwise_board.white_to_move ? "BLACK WINS" : "WHITE WINS",
             SQUARE_SIZE * 8 + x_offset, screenHeight / 2 + font_size + 5, 10,
             BLACK);
  } else if (game_state == GameStates::DRAW) {
    DrawText("DRAW", SQUARE_SIZE * 8 + x_offset, screenHeight / 2, font_size,
             BLACK);
  }
}