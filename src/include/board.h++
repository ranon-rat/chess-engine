...
public:
    ...
    std::vector<BoardCoordinates> GetMoves(BoardCoordinates piece, const BitWiseBoard &board,std::optional<bool> is_white=std::nullopt, TypeFilter filter = Legal);  // esto deberia de retornarme un uint64_t con los movimientos legales de la pieza
...
private:
    ...
    void LineMoves(Pieces piece, BoardCoordinates origin, const BitWiseBoard &board, std::vector<BoardCoordinates> &moves,bool is_white, TypeFilter filter = Legal);
    // king, knight
    void OneLineMoves(Pieces piece, BoardCoordinates origin, const BitWiseBoard &board, std::vector<BoardCoordinates> &movesbool,bool is_white, TypeFilter filter = Legal);
    // only for the pawn
